...
class page_home extends Page {
	...
	public function execute() {
		...
		$this->add_javascript("/static/widgets/page_header.js");
		$this->onload("new page_header('training_education_header');");
		$this->onload("new splitter_vertical('training_education_split',0.25);");
		$this->onload("new vertical_layout('training_education_page');");
		...
		?>
		<div id='training_education_page' style='width:100%;height:100%'>
			<div id='training_education_header' icon='/static/students/student_32.png' title='Training & Education''>
				<?php if (PNApplication::$instance->user_management->has_right("manage_batches")) { ?>
				<div class='button' onclick="create_new_batch();"><img src='/static/application/icon.php?main=/static/students/batch_16.png&small=<?php echo theme::$icons_10["add"];?>&where=right_bottom'/>New batch</div>
				<?php } ?>
			</div>
			<div layout="fill">
				<div id='training_education_split' style='width:100%;height:100%'>
					<div id='students_tree' style='overflow:auto;cursor:default'>
					</div>
		...
		var lock = lock_screen(null,"Loading...");
		...
		function build_item_element(icon, name, text, cell, onclick, oncontextmenu) {
			...
			if (oncontextmenu) {
				...
				icon.src = "/static/students/arrowDown.gif";
		...
		function build_batch_tree(batch, parent_item) {
			var batch_element = build_item_element(
				...
				function(elem, menu) {
					if (manage_batches) {
						menu.addIconItem(theme.build_icon("/static/students/academic_16.png",theme.icons_10.add,"right_bottom"), "Add Academic Period", function() { new_academic_period(elem.batch); });
						menu.addSeparator();
		...
		function build_period_tree(batch, period) {
			var period_element = build_item_element(
				theme.build_icon("/static/students/hat.png", "/static/students/calendar_10.gif", "right_bottom"),
				"Period",
				period.name,
		...
		function build_spe_tree(period, spe) {
			var spe_element = build_item_element(
				"/static/students/curriculum_16.png",
				"Specialization",
				spe.name,
		...
		function show_tabs(list) {
			...
			for (var i = 0; i < list.length; ++i) {
				...
				case "Curriculum": pages.addTab("Curriculum", "/static/students/curriculum_16.png", curriculum_frame); break;
				}
			}
		}
		...
		function select_batch(batch) {
			...
			curriculum_frame.src = "/dynamic/students/page/curriculum?batch="+batch.id;
				...
				p.addOkCancelButtons(function(){
					...
					var add_spe = function(spe) {
						...
						var add_spe_to_period = function(period_index) {
							...
							service.json("students","add_period_specialization",{period:periods[period_index].id,specialization:spe.id},function(res){
		...
		function new_class(period, spe) {
			require("popup_window.js",function() {
				...
				var p = new popup_window("Add Class",theme.build_icon("/static/students/batch_16.png",theme.icons_10.add,"right_bottom"),content);
				p.addOkCancelButtons(function(){
					if (input.value.length == 0) {
						alert("Please enter a name");
						return;
					...
					var add_to_period = function(period_index) {
						...
						service.json("students","new_class",{period:periods[period_index].id,specialization:spe ? spe.id : null,name:input.value.trim()},function(res){
