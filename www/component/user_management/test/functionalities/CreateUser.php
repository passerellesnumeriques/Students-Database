...
class CreateUser_Create extends TestFunctionalitiesStep {
	...
	public function run(&$scenario_data) {
		...
		$res = SQLQuery::create()->bypass_security()->select("Users")->where("domain","Test")->where("username","test_user")->execute();
...
class CreateUser_Remove extends TestFunctionalitiesStep {
	...
	public function run(&$scenario_data) {
		...
		$res = SQLQuery::create()->bypass_security()->select("Users")->where("id",$scenario_data["user_id"])->execute();
