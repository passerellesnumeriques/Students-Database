...
if ($batch <> null) {
	...
	for ($i = 0; $i < $conf["periods_number"]; $i++) {
		echo "periods.push({id:-1,name:".json_encode($conf["period_name"]." ".($i+1)).",academic_period:-1});";
	}
...
var graduation_date = <?php
if ($batch <> null) echo json_encode($batch["end_date"]);
else {
	$now
}
echo $batch <> null ? json_encode($batch["end_date"]) : "dateToSQL(new Date())";?>;
