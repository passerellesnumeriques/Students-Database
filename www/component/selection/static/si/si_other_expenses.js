function si_other_expenses(section, list, applicant_id, can_edit) {
	...
	this.createRow = function(item) {
		...
		if (can_edit) {
			...
			expense.onchange.add_listener(function() { item.expense = expense.getCurrentData(); });
		...
		if (can_edit) {
			...
			expense_freq.onchange.add_listener(function() { item.expense_freq = expense_freq.getCurrentData(); });
	...
	if (can_edit)
		this.save = function(ondone) {
			var locker = lock_screen(null, "Saving Other Expenses...");
