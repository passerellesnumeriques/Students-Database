...
class page_interview_center_profile extends SelectionPage {
	...
	public function executeSelectionPage() {
		...
		else $editable = $id == null || PNApplication::$instance->user_management->hasRight("manage_interview_center");
		...
		center_popup.addIconTextButton(theme.icons_16.remove, "Remove this interview center", "remove", function() {
			confirmDialog("Are you sure you want to remove this interview center ?",function(res){
				if(res){
					center_popup.freeze();
					service.json("selection","interview/remove_center",{id:<?php echo $id;?>},function(r){
						...
						window.top.status_manager.addStatus(new window.top.StatusMessage(window.top.Status_TYPE_OK, "Interview center succesfully removed!", [{action:"close"}], 5000));
						window.pnapplication.cancelDataUnsaved();
						<?php if ($onsaved <> null) echo "window.frameElement.".$onsaved."();"?>
						center_popup.close();
					});
