...
class page_import_exam_subject extends selection_page {
	...
	public function execute_selection_page(&$page) {
		...
		if(!$can_update[0]){
			...
			import_data(
				$page,
				"/static/application/icon.php?main=/static/selection/exam_32.png&small=".theme::$icons_16["add"]."&where=right_bottom",
				"Import Exam Subject",
				$data_list,
				$fixed_data,
				$prefilled_data,
				"<img src='".theme::make_icon("/static/selection/exam_16.png",theme::$icons_10["add"])."'/> Import Exam",
				"finishImport"
			);
			?>
			...
			function import_exam_subject(){
				...
				function import_exam_subject_part_2(exam_name, parts_number){
					...
					t2._init = function(){
						...
						t2.pop = new popup_window(
							...
							theme.build_icon("/static/selection/exam_16.png",theme.icons_10.add,"right_bottom"),
							t2.table,
							true
						);
				...
				function import_exam_subject_part_1(){
					...
					t1._onCancel = function(){
						...
						alert(service.generateInput(t1.res));
				...
				t._end = function(){
					...
					service.json("selection","save_exam",{exam:subject},function(res){
						...
						if(!res)
							...
							location.assign("/dynamic/selection/page/exam_subject?id="+res.id);
					});
				}
