<?php 
class service_si_save_health extends Service {
	
	public function getRequiredRights() { return array("edit_social_investigation"); }
	
	public function documentation() { echo "Save health problems"; }
	...
	public function execute(&$component, $input) {
		...
		$existing = SQLQuery::create()->select("SIHealth")->whereValue("SIHealth","applicant",$applicant_id)->execute();
		$to_insert = array();
		$to_update = array();
		$given_ids = array();
		...
		if (count($to_update) > 0)
			SQLQuery::create()->updateByKeys("SIHealth", $to_update);
		if (count($to_insert) > 0)
			$new_ids = SQLQuery::create()->insertMultiple("SIHealth", $to_insert);
		if (count($existing) > 0)
			SQLQuery::create()->removeRows("SIHealth", $existing);
