...
if (window == window.top) {
	window.top.help_system = {
		...
		activateHelp: function() {
			if (this._avail_help.length == 0) {
				infoDialog("Unfortunately, there is no help available on this page.<br/>Something is not clear ? Please <a href='/dynamic/application/page/send_feedback' target='application_frame' onclick='get_popup_window_from_element(this).close();'>write a feedback</a>");
				return;
			}
			var ids = [];
			for (var i = 0; i < this._avail_help.length; ++i)
	...
	window.init_help_system = function() {
		...
		window.top.pnapplication.onwindowclosed.addListener(function(c) {
